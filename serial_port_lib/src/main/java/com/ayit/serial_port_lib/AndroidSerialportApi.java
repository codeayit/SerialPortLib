...
//                                    Log.d(TAG,"实时接受：:" +ByteArrToHex(bos.toByteArray()));
                                    ...
                                    if (--count == 0) {
                                        if (bos.size() > 0) {
                                            ...
                                            bos = null;
                                            if (observer != null) {
                                                final Map<Byte,List<byte[]>> data = observer.onProcessInSubThread(bytes);
                                                for (Map.Entry<Byte,List<byte[]>> entry:data.entrySet()){
...
//                                                observer.onObserve(data);
    ...
    public interface ReadDateObserver {
         ...
         * @return
         */
        Map<Byte,List<byte[]>> onProcessInSubThread(byte[] data);
         ...
         * @return
         */
        void onObserveInMainThread(Map<Byte,List<byte[]>> data);
    ...
    public static short hex2short(String hexH,String hexL) throws Exception {
        ...
        short i =binaryToShort(byte2BinaryString(HexToByte(hexH))+byte2BinaryString(HexToByte(hexL)));
        short shortI = (short)i;
        return shortI;
    }

    public static String[] short2hex(short number){
        String hexH = Byte2Hex((byte)(number>>8));
        String hexL = Byte2Hex((byte)((number<<8)>>8));
